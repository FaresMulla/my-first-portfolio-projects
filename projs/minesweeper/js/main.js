...
// @CR - Hey Fares! First of all great job! I loved what you've done!
...
function init() {
    ...
    // @CR - By convention, var names for DOM elements start with 'el', names here should be : elImojeGame, elLives, elLamps.
    var imojeGame = document.querySelector('.middelDiv')
    imojeGame.innerHTML = HAPPY

    // @CR - The vars 'LIFE', 'LAMP' and 'CHECK' type is arrays. When you render it the user see ',' between the elements. There are ways to avoid it!
    ...
    // @CR - Just made a few changes here to make it work :)
...
// @CR - Always define global vars at the start of document
var ginterval;
...
// @CR - Keep it simple! if the function name is 'createBoard', this is the only thing it should do. For the rest of the actions, you can write additional functions such as startTimer(), placeMines().
function createBoard(size = 4, mines) {
    var elTime = document.querySelector('.timeDiv')
    var counterTime = 0
    ginterval = setInterval(() => {
        counterTime++
        elTime.innerText = 'Time: ' + `${Math.floor((counterTime / 60))}:${("0" + counterTime).slice(-2)}`
    }, 1000)
    ...
    // @CR - Placing mines this way you can get a location more then once and have less mines then you expected.
...
function renderBoard(gBoard) {
    ...
    // @CR - No need to define the var 'cell' here, you can define it at the first time you use it.
    var cell = {}
    var strHTML = '';
    for (var i = 0; i < gBoard.length; i++) {
        ...
        strHTML += `</tr>\n`
        // @CR - Clean commented lines before submittion
        // console.log('strHTML ', strHTML);
    }
    // fixed proplem in the consol!

    // @CR - I didn't understand what is the purpose of the following code (inside setTimeout). notice that: 'elTd' here is the first html table cell, and 'cell' here is the last gBoard cell.
    setTimeout(() => {
...
// @CR - Better names can be 'setContent', Good names for functions are usualy actions. Also, the right name for the var 'pos' is 'cell'.
...
function cellClicked(elCell, i, j) {
    ...
    if (cell.isMarked) {
        cell.isMarked = false
        // @CR - No need to render all board now. you can render only the clicked cell, to find it use 'elCell'.
        renderBoard(gBoard)
        return
    }
...
function checkWinner() {
    ...
    if (counterWinner >= counter) {
        ...
        var theWinnerC = JSON.parse(localStorage.getItem('winnerGame'))
        // @CR - You repeat all actions you made inside the 'if' scop, inside the 'else if' scope. You can improve your code here.
        if (!localStorage.length > 0) {
...
// @CR - Better names can be 'GetMinesAroundCount'
